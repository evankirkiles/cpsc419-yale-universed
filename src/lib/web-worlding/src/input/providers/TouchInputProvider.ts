...
export default class TouchInputProvider implements IInputProvider {
  ...
  constructor(
    ...
    ].reduce<typeof this.buttonMapping>((acc, button) => {
  ...
  async listen() {
    ...
    this.nippleManager = (
      require("nipplejs").default as typeof NippleJs
    ).create({
    ...
    Object.values(this.buttonMapping).forEach(
      ({ domElement, boundTouchStartListener, boundTouchEndListener }) => {
        this.domElement.append(domElement);
        domElement.addEventListener(
          "touchstart",
          boundTouchStartListener,
          false
        );
